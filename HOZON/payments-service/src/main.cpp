...
using namespace hozon::payments;
...
int main() {
    std::cout << "=== Payments Service ===" << std::endl;
    ...
    try {
        // Конфигурация
        int httpPort = config::getEnvInt("HTTP_PORT", 8082);
        std::string dbHost = config::getDbHost();
        int dbPort = config::getDbPort();
        std::string dbName = config::getEnv("DB_NAME", "payments");
        std::string dbUser = config::getEnv("DB_USER", "payments");
        std::string dbPassword = config::getEnv("DB_PASSWORD", "payments");
        ...
        auto repo = std::make_shared<AccountRepository>(db);
        ...
        InboxWorker inboxWorker(repo, consumer);
        OutboxWorker outboxWorker(repo, publisher);

        // Запуск воркеров
        inboxWorker.start(queues::PAYMENT_REQUESTS);
        outboxWorker.start(queues::PAYMENT_RESULTS);

        // Инициализация HTTP сервера
        HttpServer server(httpPort);
        PaymentHandler handler(repo);
        handler.registerRoutes(server);

        // Запуск HTTP сервера
        server.start();

        std::cout << "Payments Service is running on port " << httpPort << std::endl;
        ...
        std::cout << "Payments Service stopped." << std::endl;
        return 0;
