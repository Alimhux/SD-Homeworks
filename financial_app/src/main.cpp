...
public:
    Money(Decimal amount, const std::string& currency = "RUB")
    ...
    Money subtract(const Money& other) const {
        if (currency_ != other.currency_) {
            throw ValidationException("Cannot subtract money with different currencies");
        }
        return Money(amount_ - other.amount_, currency_);
    }
    ...
    bool operator<(const Money& other) const {
        if (currency_ != other.currency_) {
            throw ValidationException("Cannot compare money with different currencies");
        }
